...
SET start=%time%
ECHO ##########################################################
ECHO #		xCP Tomcats Host Start
ECHO #		Author: Tom Brennan
ECHO #		Client: HMRC
ECHO #		Date Started: 08/09/2016
ECHO #		Last Updated Started: 08/09/2016
ECHO #		Last Updated by: TB
ECHO #		Information: This script starts all Application Host xCP Components Services and clears all caches		
...
for /F "tokens=2 delims==" %%s in ('SET APPLICATION.SERVER.HOST.') do (
	...
	ECHO Starting %%s BAM
	call safeServiceStart.bat %%s %APPLICATION.SERVER.NAME.BAM%
	
	ECHO Starting %%s BPS
	call safeServiceStart.bat %%s %APPLICATION.SERVER.NAME.BPS%


	ECHO Starting %%s XMS
	call safeServiceStart.bat %%s %APPLICATION.SERVER.NAME.XMS%
