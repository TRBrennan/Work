...
for /F "tokens=2 delims==" %%s in ('SET CONTENT.SERVER.HOST.') do (
	...
	call safeServiceStop.bat %%s DctmThumbNailServer
	call safeServiceStop.bat %%s DmMethodServer
	call safeServiceStop.bat %%s DmDocBrokerPrivate
	call safeServiceStop.bat %%s DmDocBroker
	call safeServiceStop.bat %%s DmServerDMS	
)
