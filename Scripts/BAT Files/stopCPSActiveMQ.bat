...
SET start=%time%
ECHO ##########################################################
ECHO #		CPS ActiveMQ Stop
ECHO #		Author: Tom Brennan
ECHO #		Client: HMRC
ECHO #		Date Started: 08/09/2016
ECHO #		Last Updated Started: 08/09/2016
ECHO #		Last Updated by: TB
ECHO #		Information: This script stops all Application Host CPS and Active MQ Services and clears all caches		
...
for /F "tokens=2 delims==" %%s in ('SET APPLICATION.SERVER.HOST.') do (	
	...
	ECHO Stopping Active MQ and CPS
	call safeServiceStop %%s ActiveMQ
	call safeServiceStop.bat %%s %APPLICATION.SERVER.NAME.ACTIVEMQADMIN%	
	call safeServiceStop.bat %%s %APPLICATION.SERVER.NAME.CPS%
	ECHO DEL DIR \\%%s\%APPLICATION.SERVER.FOLDER.PATH%%APPLICATION.SERVER.NAME.CPS%\temp
	RMDIR \\%%s\%APPLICATION.SERVER.FOLDER.PATH%%APPLICATION.SERVER.NAME.CPS%\temp /q /s
	ECHO DEL DIR \\%%s\%APPLICATION.SERVER.FOLDER.PATH%%APPLICATION.SERVER.NAME.CPS%\work
	RMDIR \\%%s\%APPLICATION.SERVER.FOLDER.PATH%%APPLICATION.SERVER.NAME.CPS%\work /q /s
	MKDIR \\%%s\%APPLICATION.SERVER.FOLDER.PATH%%APPLICATION.SERVER.NAME.CPS%\work
	MKDIR \\%%s\%APPLICATION.SERVER.FOLDER.PATH%%APPLICATION.SERVER.NAM.CPS%\temp
)
