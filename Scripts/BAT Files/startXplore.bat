...
::Set xPlore Servers
SET XPLORE.SERVER.HOST.1=10.21.176.150
SET XPLORE.SERVER.HOST.2=10.21.176.151
...
for /F "tokens=2 delims==" %%s in ('SET XPLORE.SERVER.HOST.') do (

		taskkill /s %%s /im monitoringhost* /f
	
		call safeServiceStart.bat %%s DmWatchdog-E-xPlore
		call safeServiceStart.bat %%s DmPrimaryDsearch
		call safeServiceStart.bat %%s DmIndexagent
	)
