...
::Set Content Servers
SET CONTENT.SERVER.HOST.1=10.21.176.154
SET CONTENT.SERVER.HOST.2=10.21.176.156
SET CONTENT.SERVER.HOST.3=10.21.176.150
SET CONTENT.SERVER.HOST.4=10.21.176.151
...
:: Stopping CONTENT Services
for /F "tokens=2 delims==" %%s in ('SET CONTENT.SERVER.HOST.') do (
	taskkill /s %%s /im monitoringhost* /f
	call safeServiceStart.bat %%s DmDocBrokerPrivate
	call safeServiceStart.bat %%s DmDocBroker
	call safeServiceStart.bat %%s DmServerDMS
	call safeServiceStart.bat %%s DmMethodServer
	REM call safeServiceStart.bat %%s DctmThumbNailServer
)
