...
ECHO #		Information: This script stops  all Application Host Services and clears all caches		
...
SET APPLICATION.SERVER.NAME=tomcat-dms-
...
for /F "tokens=2 delims==" %%s in ('SET APPLICATION.SERVER.HOST.') do (
	for /F "tokens=2 delims==" %%a in ('SET APPLICATION.SERVER.PORT.') do (
		...
		ECHO DEL DIR \\%%s\%APPLICATION.SERVER.FOLDER.PATH%%APPLICATION.SERVER.NAME%%%a\temp
		RMDIR \\%%s\%APPLICATION.SERVER.FOLDER.PATH%%APPLICATION.SERVER.NAME%%%a\temp /q /s
		ECHO DEL DIR \\%%s\%APPLICATION.SERVER.FOLDER.PATH%%APPLICATION.SERVER.NAME%%%a\work
		RMDIR \\%%s\%APPLICATION.SERVER.FOLDER.PATH%%APPLICATION.SERVER.NAME%%%a\work /q /s
		MKDIR \\%%s\%APPLICATION.SERVER.FOLDER.PATH%%APPLICATION.SERVER.NAME%%%a\work
		MKDIR \\%%s\%APPLICATION.SERVER.FOLDER.PATH%%APPLICATION.SERVER.NAME%%%a\temp
	)
