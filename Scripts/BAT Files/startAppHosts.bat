...
SET start=%time%
ECHO ##########################################################
ECHO #		DMS Application Host Start
ECHO #		Author: Tom Brennan
ECHO #		Client: HMRC
ECHO #		Date Started: 08/09/2016
ECHO #		Last Updated Started: 08/09/2016
ECHO #		Last Updated by: TB
ECHO #		Information: This script Starts all Application Host Services and clears all caches		
...
for /F "tokens=2 delims==" %%s in ('SET APPLICATION.SERVER.HOST.') do (
	for /F "tokens=2 delims==" %%a in ('SET APPLICATION.SERVER.PORT.') do (
		...
		call safeServiceStart.bat %%s %APPLICATION.SERVER.NAME%%%a
		ECHO %APPLICATION.SERVER.NAME%%%a on host %%sp
	)
