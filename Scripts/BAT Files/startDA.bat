...
ECHO #		Information: This script starts all Application Host DA Services and clears all caches		
...
SET APPLICATION.SERVER.HOST.1=10.21.176.46
SET APPLICATION.SERVER.HOST.2=10.21.176.47
...
for /F "tokens=2 delims==" %%s in ('SET APPLICATION.SERVER.HOST.') do (	
	...
	ECHO Starting %%s BISDA
	call safeServiceStart.bat %%s %APPLICATION.SERVER.NAME.BISDA%

	ECHO Starting %%s DA
	call safeServiceStart.bat %%s %APPLICATION.SERVER.NAME.DA%
)
